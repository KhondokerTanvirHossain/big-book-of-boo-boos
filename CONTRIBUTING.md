...
- **The line-count rule.** Big Book's custom code is capped at 5–10k lines total. Every glue PR states its net line change in the description. ~~Issue #7 has a hard limit of 1.6k~~ — **retired for #7 on 2026-09-24**, once the enforcement path had closed at 1,684 and the ADR-001 deferred re-check (+≈60) was ruled in: §3.1 records #7 as the number rather than against a ceiling. **The per-issue rule is unchanged for every other issue, and the 5–10k total is the live control.** Others report. **If an issue's actual exceeds its [ARCHITECTURE §3](docs/ARCHITECTURE.md#3-module-map) share (per issue: [§3.1](docs/ARCHITECTURE.md#31-per-issue-glue-estimate)), the PR stops for a decision.** Lines for work already inside the v0.1 estimate are actuals against that share and need no matching removal; the "sign-off line reached" note in `docs/BIGBOOK.md` is about *new scope*, which does need one. If you find yourself writing something HAPI or Keycloak already does, stop — the answer is configuration, not code.
- **A print is not a measurement. Assert, or the run proves nothing.** A `System.out.println` of a result records what happened once; it does not fail when the answer changes, and nobody reads it on a green run. On #12, ADR-002 Open 2's first run printed `VERIFY-A2 POST arrived = false; bodies = 0` and the suite reported **3 tests, 0 failures** — the verify had measured a "no" on a question whose answer decides a ~150-line estimate, and said so only in scrollback. The cause turned out to be a missing collaborator in the probe, so the real answer was **yes**; had that print been trusted, the issue would have been re-planned around a wrong finding. Where a verify's answer matters, assert it: the same test now asserts arrival, that the body is the resource, and that HAPI's `User-agent` did the POST. Print alongside the assertion if it helps a reader, never instead of it.
- **A control that cannot run is not a control.** Assert that the control produced its expected *positive* before trusting the negative beside it. On #12 the measurement was "HAPI's queue received 0 messages when the override skips `super`" — which means nothing unless `super` demonstrably *does* reach the queue. The control NPE'd three times running (on `mySubscriptionSettings`, then on the persistence service), and throughout, the negative kept reporting `consumer received 0` and looking like a pass while the suite showed 3 of 4 green. Only once the control returned `persistence saw 1` was the zero worth anything. If a test's meaning depends on a sibling test passing, assert the sibling's positive result, not just its absence of failure.
- **A passing test proves the outcome, not the reason.** Where a rule exists to force a *specific* refusal, assert **which** rule refused. On #36 the resolver's fail-closed rule — a conditional reference resolving to zero matches must refuse — was planted away by returning the raw match URL instead of `null`, and **all seven existing tests still passed**: the unresolved string fails the criterion anyway, so the write was refused for the wrong reason. The test that catches it asserts the `OperationOutcome` carries the *resolver's* message ("could not be checked against the caller's access policy") rather than the criteria's ("outside the criteria of the caller's access policy"). Two rules that both refuse are not interchangeable; if only one of them is meant to fire, say so in the assertion.
- **A green banner is not a green run. Assert on produced test artefacts, not on build output.** On #9 a `./gradlew test --rerun-tasks` printed `BUILD SUCCESSFUL in 1m 31s` while **silently skipping `:server:test` altogether** — `:core:test` ran, the server suite did not appear in the task list at all, and the stale XML from an earlier run was still on disk to make the directory look populated. The green was real and meant nothing. Before reporting a suite as passing, count what it produced: the number of `TEST-*.xml` files, the `tests=`/`failures=` totals inside them, and that the classes you care about are among them. Delete `build/test-results` first if the run's honesty matters. Relatedly, never run two `:server:test` invocations concurrently — they corrupt Gradle's results directory (`NoSuchFileException: in-progress-results-generic.bin`) and report a *build* failure that is not a test failure, which wastes a diagnosis.
- **A fill marked `wire` still needs one request per feature.** `wire` says the component is capable, not that it behaves by default. On #9, three features BB-R-002 specifies as wire were absent (`_filter`, `HAPI-1222`), broken (`:missing` — malformed SQL and a 500, not a 400) or refused (`:contains`, 405) on stock settings. See `docs/guides/medplum-parity.md` → "Wire means the component *can* do it".
- **Coverage by enumerated values is not coverage by request shape.** A guard that dispatches on a request attribute needs a test for the **absence** of that attribute. #34 added `deny` rules per project-admin type and tested every one of them; each test made a single *typed* request, so nothing exercised a request with no resource type. HAPI's `resourcesOfType` deny **matches** a typeless request rather than abstaining, and following any paging link came back `403 "T1: * does not cover Cron"` — shipped, and found only when #9 exercised `_count` and `next`. Every interaction was covered; a typeless request is a *shape*, not an interaction. Enumerate the shapes the dispatch can see, including the empty one.
