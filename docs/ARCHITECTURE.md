...
| #7 access policy | — | **1,684 actual** | **358 actual** `AccessPolicy` provider and wiring (g), against ≈150 | — | **2,042 actual** | ADR-001 as amended 2026-09-21 (≈1.45–1.65k, including the deferred re-check ~60). **The per-issue ceiling is retired for #7** (ruled 2026-09-24): the enforcement path is recorded as the number, 1,684, not as a figure against 1,680. It closed 4 over, all of it the `fullyResolved` deferral fixing a phase-1 over-refusal found after the ceiling was set; a further **+≈60 is the ADR-001 deferred phase-2 re-check**, ruled 2026-09-21 and not yet built. **CONTRIBUTING §4's per-issue rule is unchanged for every other issue; the 10k tripwire is the live control.** ADR-001's ≤200 verification-dependent extras went unused — write-check verifies (d) and (e) both came back yes (2026-09-23). The provider counts against its own column, not the path — see §3.2 for its split, which re-prices the five ADR-003 admin-type rows |
