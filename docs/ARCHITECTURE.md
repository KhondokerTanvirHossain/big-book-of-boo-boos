...
What CONTRIBUTING §4 compares a PR against: *if an issue's actual exceeds its share here, the PR stops for a decision.* Derived from the §3 module shares and the ADR budgets. **Source** says where a number comes from; **(g)** marks a split nobody has decided, made so that each module's rows add up to its §3 share. A guessed figure is a tripwire, not a target: crossing it starts a conversation. Types whose job is to make an illegal state unrepresentable — `CompiledPolicy` records, a sealed `Criteria` hierarchy, ArchUnit rules — are **not counted here**, on the same footing as tests (CONTRIBUTING §4). The test is whether removing the line would make an invalid state representable: a line that decides *who gets access* is counted wherever it lives, so `PolicyDefaults` (super-admin bypass, full project access, `denyAll`) counts while `CompiledPolicy`'s invariants do not (ruled 2026-09-22). **Module shares are indicative; the total and the per-issue rows are what the CONTRIBUTING §4 rule tests against.** Lines are main Java only; tests, SQL, YAML and shell are uncounted. Replace an estimate with the actual when the issue merges.

**These estimates price the work believed remaining, and measurement corrects them in both directions.** An estimate is a claim about what is left to build, made before anyone looked; two issues have already moved it the opposite way from each other. #9's `_project`/`_compartment` share (T26, ≈50) turned out to be **already built** in #4, so the priced item cost zero and the actual went instead to work the estimate did not know about. #12's subscription matcher was **assumed present** — ADR-002 prices the delivery table at ~150 on top of "HAPI matching" — and measurement found HAPI's submit and match configs are not registered at all, so #12 wires the matcher as well. A share exceeded is a prompt to ask which of the two happened, not automatically a sign the work overran.
...
| #5 auth surface | — | — | **420 actual**: Keycloak login/asset proxy 119 + its negative allow-list filter 61, `/oauth2/*` and the reshaped discovery/logout 139, `/auth/me` 62, security config +39 | — | **420 actual** | PR for #5. Estimate was ≈310 (≈280 remainder + 30 for the proxy), so +110 over; **accepted against `server/` by the maintainer 2026-09-20.** Where it went: the proxy's negative half (61) was not estimated at all — the allow-list needs a filter that answers 404 for Keycloak's other paths, which the 30-line estimate did not anticipate; and the proxy itself is 119, not 30, because forwarding headers, passing `Set-Cookie`/302 through and streaming bodies is more than a redirect. Deferred, so not counted here: the v0.2 `/oauth2/token` response wrapper (D3) |
| #6 invite, client routes | **249 actual** | — | **253 actual** | — | **502 actual** | PR for #6. Estimate was ≈570 (370 tenant + 200 server), so **68 under**. The split was also close: 249 in `core/` against 370 guessed, 253 in `server/` against 200 — the profile-resource writer (113) and the invite payload mapping (131) live in `server/`, next to HAPI and the request. Not counted, deferred to #11: the SMTP wiring behind `sendEmail` |
| #7 access policy | — | **1,684 actual** | **358 actual** `AccessPolicy` provider and wiring (g), against ≈150 | — | **2,042 actual** | ADR-001 (≈1.4–1.6k). **The per-issue ceiling is retired for #7** (ruled 2026-09-24): the enforcement path is recorded as the number, 1,684, not as a figure against 1,680. It closed 4 over, all of it the `fullyResolved` deferral fixing a phase-1 over-refusal found after the ceiling was set. **CONTRIBUTING §4's per-issue rule is unchanged for every other issue; the 5–10k total is the live control.** ADR-001's ≤200 verification-dependent extras went unused — write-check verifies (d) and (e) both came back yes (2026-09-23). The ADR-001 deferred phase-2 re-check was **ruled out on 2026-09-24** and replaced by a match-URL resolver, **74 actual against ≈60** (ruled 2026-09-22 on #36; accepted 2026-09-24). The decision priced it as net ≈0 — −60 for reverting the deferred path, +60 for the resolver — but **the −60 was phantom: the deferred path was never built**, having been ruled out on #35 before any code existed, so there was nothing to revert and the true figure is +74. Why the resolver is needed: `PRECOMMIT` substitutes `urn:uuid` placeholders but leaves **conditional** references raw — two mechanisms, and the #35 report generalised from one to both (corrected on #36). **Post-merge follow-ups charged here, not to the issues that found them:** +52 for T1 at the authorization layer (#34 — `Entry.covers` enforced T1 in the criteria hooks while the rule list handed out `allResources()`), and +10 for the paging 403 that #34's deny rules caused (found by #9). The provider counts against its own column, not the path — see §3.2 for its split, which re-prices the five ADR-003 admin-type rows |
...
| #9 search | — | — | **≈92 actual** (accepted 2026-09-24) against ≈50 | — | **≈92** | The ≈50 priced only `_project`/`_compartment` → partition (T26), **which cost 0** — already built in #4, and the duplicate started for #9 was deleted. So the estimate was wrong about the issue's *contents*, not overrun by the work. The actual is the two items the verify-first block uncovered: `_summary` in paging links ≈65 (D17 — HAPI's `_getpages` cursor drops it, so page 2 returns full untagged resources and poisons `@medplum/core`'s cache per D44; named in BB-R-002 and in #9's own exit test, so not splittable) and routing HAPI's non-partitionable types to the default partition ≈22 (V4 — runtime `SearchParameter` was blocked by `422 HAPI-1318` until then). A further 10 lines fixing a paging 403 are **charged to #34**, whose deny rules caused it |
...
### 3.2 The `AccessPolicy` provider's 358 (2026-09-23)

Recorded because ADR-003 reuses this pattern for five more Medplum admin types in v0.2 (`Project`,
`ProjectMembership`, `ClientApplication`, `User`, `UserConfiguration` — inventory area 1), and what that costs
depends on how much of the first one is reusable rather than `AccessPolicy`-specific.

| Part | Lines | Reusable for the next admin type? |
|---|---|---|
| `AccessPolicyResource` — the `@ResourceDef` class | 62 | **Pattern only.** Each type needs its own class; the shape (one `@Child`, `copy`, `fhirType`, null `getResourceType`) transfers, roughly 40 of the 62 being boilerplate that differs only in the name |
| `AccessPolicyStore` — CRUD + project scoping | 133 | **≈90 reusable.** The four statements, the project-scoped `WHERE`, the 404-not-403 contract and the JSON parse/fail-closed path are the same for every type. `load(projectId, references)` with its version list (≈46) is policy-specific: only `AccessPolicy` is fetched by reference from a membership |
| `AccessPolicyProvider` — `@Create`/`@Update`/`@Read`, project resolution | 160 | **≈95 reusable.** `projectOf`, `idOf`, `parse`, the create/update/read skeleton and the id-and-body wiring are per-type identical. The ≈65 of criteria validation (`validateCriteria`, `outcome`, the `CriteriaRejectedException` → 400 mapping) exists only because `AccessPolicy` carries `criteria` |
| V4 migration | 25 | **Pattern only.** One table per type, same three columns plus `document jsonb` |
| **Total** | **383** | **≈185 transfers as code, ≈150 as pattern; ≈48 is `AccessPolicy`-only** |

Re-pricing the ADR-003 rows: the first admin type paid the pattern cost. A second type is closer to
**≈110–130** (its `@ResourceDef` class, its table, and the type-specific half of its provider) rather than
another 358, provided the reusable ≈185 is factored into a shared base as the second type is written. Factoring
it out *before* the second type would be speculative — one instance is not a pattern.

The 358 in the table above is the three Java files as measured by the line counter; the 383 here includes the
25-line migration, which the counter attributes to the same row.
