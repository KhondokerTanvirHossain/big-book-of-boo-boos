...
| #5 auth surface | — | — | **420 actual**: Keycloak login/asset proxy 119 + its negative allow-list filter 61, `/oauth2/*` and the reshaped discovery/logout 139, `/auth/me` 62, security config +39 | — | **420 actual** | PR for #5. Estimate was ≈310 (≈280 remainder + 30 for the proxy), so **+110 over**, and by CONTRIBUTING §4 this PR stops for a decision. Where it went: the proxy's negative half (61) was not estimated at all — the allow-list needs a filter that answers 404 for Keycloak's other paths, which the 30-line estimate did not anticipate; and the proxy itself is 119, not 30, because forwarding headers, passing `Set-Cookie`/302 through and streaming bodies is more than a redirect. Deferred, so not counted here: the v0.2 `/oauth2/token` response wrapper (D3) |
