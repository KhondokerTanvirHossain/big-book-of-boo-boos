...
| #7 access policy | — | 1.4–1.6k | **358 actual** `AccessPolicy` provider and wiring (g), against ≈150 | — | ≈2.04k | ADR-001 line items: translator 400, `PRESEARCH` 120, `PREACCESS` 150, `hiddenFields` 150, `readonlyFields` 100, post-write 50, write-time validation 60, params 100, interaction split 60, subscriptions 100, denial log 50, backstop 20 = 1,360, + extras ≤200. **Ceiling 1.68k on the enforcement path** (`core/policy` + `server/policy` less the provider; issue #7, raised from 1.6k on 2026-09-22, the extra 80 attributed to the V5 fail-open rules). **Actual 1,684 — 4 lines over.** The overrun is entirely the `fullyResolved` deferral (+20, partly absorbed by consolidation), which fixes a phase-1 over-refusal found *after* the ceiling was set: at `PRESTORAGE` a conditional reference still reads `Patient?identifier=…` and fails every criterion, including the ones it would satisfy once resolved, so legitimate transactions got a 403. Reported rather than shaved — the remaining candidates are the package-private `PolicyContext` seam and comment text (issue #7). ADR-001's ≤200 verification-dependent extras went unused: write-check verifies (d) and (e) both came back yes (2026-09-23). **The provider counts against this row, not the ceiling** (ruled 2026-09-23) — see §3.2 for its split, which re-prices the five ADR-003 admin-type rows |
