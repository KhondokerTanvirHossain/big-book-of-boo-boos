...
- **HAPI's conditional create resolves match URLs across partitions by default**, so partition-scoped match resolution is Big Book's responsibility wherever conditional operations are enabled (found in #6: inviting one person to a second project collided on `HFJ_RES_SEARCH_URL`, because `Practitioner?identifier=<email>` in one tenant blocked the identical create in another). Anyone embedding HAPI with a partition per tenant inherits this: the setting that scopes it is off by default, and its own documentation describes the default as *preventing* duplicates across partitions — correct for de-duplication, wrong for tenancy.
