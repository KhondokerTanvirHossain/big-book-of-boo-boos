...
Status: decided (2026-09-12), amended (2026-09-17, inventory reconcile — D3/D51, D24, D49, D50/T49, T50; 2026-09-19, #5 — login proxy and token claims)
...
- Token issuer is `<BIGBOOK_BASE_URL>realms/bigbook` — Keycloak's `hostname` is set to Big Book's public URL and Keycloak always appends `/realms/<realm>` (measured on 26.7.4, issue #5); `/.well-known/openid-configuration` advertises that same string as its `issuer`; `/.well-known/openid-configuration` and `/oauth2/logout` are re-shaped by Big Book; `/oauth2/authorize`, `/oauth2/userinfo` and `/.well-known/jwks.json` proxied to Keycloak unchanged (BB-R-004 §8). **`/oauth2/token` is proxied unchanged in v0.1 and re-shaped from v0.2** (D3/D51): the SDK reads `project` and `profile` from the token response body and requires `exp`, `client_id` (or `cid`) and `login_id` claims — without `login_id` it never calls `/auth/me` and behaves as signed-out while holding a valid token. Keycloak mappers supply the claims; a response wrapper supplies the two body fields.
- (2026-09-19, #5) Big Book proxies exactly two Keycloak prefixes in both profiles: `/realms/<realm>/**` and `/resources/**` (allow-list; realm fixed from config; `/admin/**`, `/realms/master/**`, `/metrics`, `/health` → 404). Keycloak publishes no port; the admin console is reached only via `KC_HOSTNAME_ADMIN` on an operator-only address. `full` routes the same prefixes through Big Book, not Traefik→Keycloak. ~30 glue lines, same passthrough as `/oauth2/*`.
- (2026-09-19, #5) Token claims: every token carries `project` (`organization`); `ClientApplication` tokens carry `profile` and `membership` via hardcoded mappers; user tokens do not — Big Book resolves both from `(sub, project)` per request and via `/auth/me`. BB-R-004.9 reworded accordingly. The v0.2 `/oauth2/token` wrapper (D3) supplies `project`/`profile` in the body and stock mappers supply `login_id`/`client_id`; no Keycloak provider JAR is needed for Medplum compatibility at any grade. A per-membership claim (`fhirUser`, BB-R-022) is the sole remaining case and is decided in ADR-005.
