...
Exit test: super-admin creates two projects; invites the same email to both, admin in A and read-only in B; Patient created in A is invisible from B's token; `/auth/me` under each token shows the right membership; a fresh install with no `BIGBOOK_ADMIN_PASSWORD` prints a generated one and refuses `medplum_admin`.
...
Fill: **glue** (ADR-001, decided 2026-09-12, amended 2026-09-17: no external engine — Medplum-shaped `AccessPolicy` resources translated at request time into HAPI `AuthorizationInterceptor` rules + `SearchNarrowingInterceptor`; field hiding, readonly restore, parameter substitution, write-time criteria validation and subscription-side enforcement as Big Book interceptor hooks; **two-phase** write authorisation; **fails closed**; ≈1.2–1.4k lines) · Version: v0.1 · Displaces: nothing · Status: blessed
...
Verify: **V1** `SearchNarrowingInterceptor` fires for `_include`/`_revinclude` sub-queries (**blocking** — a miss is a cross-tenant leak); **V5** `InMemoryResourceMatcher` capability set.
