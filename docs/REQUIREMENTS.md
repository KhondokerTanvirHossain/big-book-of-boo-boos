...
Fill: **wire HAPI JPA** · Version: v0.1 · Displaces: nothing · Status: draft
...
Fill: **wire HAPI** · Version: v0.1 · Displaces: nothing · Status: draft
...
Fill: **wire HAPI GraphQL** · Version: v0.1 · Status: draft
...
Fill: **wire Keycloak** (+ glue for Medplum-shaped endpoints, scope per ADR-003) · Version: v0.1 · Status: draft
...
8. Medplum-shaped endpoints: `/oauth2/authorize`, `/oauth2/token`, `/oauth2/userinfo` (proxy/alias to Keycloak), `/auth/me` (**glue**: returns profile, project, membership, access policy, user config — required by the SDK and any Medplum-compatible UI). `/auth/login`, `/auth/newuser` etc. in scope only if ADR-003 says so.
...
Fill: **glue** (tenant model) on Keycloak organisations + HAPI partitions · Version: v0.1 · Displaces: nothing (HAPI has partitions, no project/membership model) · Status: draft
...
Fill: **glue** (policy adapter: Medplum-shaped `AccessPolicy` JSON → engine per ADR-001, enforced via HAPI `AuthorizationInterceptor`) · Version: v0.1 · Displaces: nothing · Status: draft
...
Fill: **wire HAPI Subscriptions** (+ glue for Medplum extensions where cheap) · Version: v0.1 · Status: draft
...
Fill: **wire n8n** via BB-R-007 rest-hooks · Version: v0.1 (recipe + example; n8n container only in `full`) · Status: draft
...
Fill: **wire HAPI binary storage** (`lite`: database/local; `full`: MinIO) · Version: v0.1 · Status: draft
...
Fill: **wire Keycloak email + Spring Mail** · Version: v0.1 · Status: draft
...
Fill: **glue** (Helm chart, compose, profiles, docs) · Version: v0.1 · Status: draft
...
Fill: **glue** (thin layer over HAPI generic client + Keycloak) · Version: v0.1 · Status: draft
...
Fill: **wire Appsmith/ToolJet** on `/fhir/R4` + `/admin` (ADR-004 decides the long-term path) · Version: v0.1 (low-code) → v0.3 (proper) · Status: draft
...
Fill: **glue**, scope fixed by **ADR-003** · Version: v0.1 (FHIR + OAuth), v1.0 (admin routes verified) · Status: draft — blocked on ADR-003
...
3. Medplum admin resource types readable/writable in Medplum JSON shape: `Project`, `ProjectMembership`, `AccessPolicy`, `ClientApplication`, `User` (read-only projection of Keycloak), `UserConfiguration`. `Bot` v0.2, `Login` non-goal.
4. Admin routes `/admin/projects`, `/admin/projects/:id`, `/admin/projects/:id/invite|client|members`.
5. Decision needed (ADR-003): whether the Medplum TypeScript SDK / React app must work unmodified against Big Book. If yes, (3)–(4) are v0.1; if FHIR+OAuth only, (3)–(4) become v0.2.

Exit test (v0.1 minimum): Medplum's own `curl` examples from `fhir-datastore/creating-data` and `api/oauth/token` work against Big Book unchanged.

## BB-R-015 Observability (minimum)
Medplum ref: `self-hosting/monitoring`, `self-hosting/opentelemetry`
Fill: **wire OpenTelemetry** (exporters on in `full`; `lite` logs to stdout) · Version: v0.1 minimum, v0.2 full stack · Status: draft
