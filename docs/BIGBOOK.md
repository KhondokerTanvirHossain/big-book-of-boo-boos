...
**v0.1 glue tally (2026-09-18): ≈5.69k estimated, 1.88k actual — sign-off line reached; further v0.1 glue requires a matching removal.** `core/` ≈2.9k (tenant model 1.3k incl. the ADR-007 `provisioning` anchor rows and startup reconciler ≈60 · policy adapter 1.4–1.6k per ADR-001 as amended incl. two-phase write check and criteria validator), `server/` ≈1.8k (OAuth passthrough + reshape, `/auth/me`, admin routes, subscription delivery table + poller + signature + AuditEvent ≈150, outbound allow-list ≈40, GraphQL limits ≈50, `X-Project`, bootstrap), `client/` ≈0.8k. Known v0.2 additions from the inventory: token-response wrapper + mappers, extended-meta interceptor, OperationOutcome slug mapping and the eight T29 alignments, `$export` poll aliasing ≈200, no-op PUT 304 ≈40, CLI Basic-auth filter ≈80, token-exchange glue ≈150 (D9, `full`). Detail per module in `ARCHITECTURE.md` §3.
